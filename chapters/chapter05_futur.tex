...
Finalement, jusqu'à présent seuls des prélèvements en air extérieur ont été considérés. Or, en moyenne, un européen passe la majeur partie de son temps en espace clos intérieur (domicile,
...
Ainsi, et en première approximation, il est possible d'estimer la contribution relative 
mensuelle moyenne des différentes sources à la masse totale des PM. 
Puis, connaissant le PO intrinsèque (i.e. par microgramme de PM) de chacune de ces sources, une estimation au premier ordre du PO est envisageable.
La conséquence est que pour chaque valeur de concentration de \PMdix{} il est possible d'estimer
une valeur de PO associée.

Formellement, cela revient à généraliser la figure 6 de \cite{weberSourceinprep.} à un site quelconque et donc à attribuer les PO par microgramme de \PMdix{} mensuels présentés dans le tableau~\ref{tab:monthly_opi}.
...
\begin{table}[ht]
    ...
    \bottomrule
    \end{tabular}
    \caption{Approximation du PO intrinsèque (i.e. par microgramme de \PMdix{} mensuel en ne prenant en compte que les 8 sources de PM majoritaires déterminées par \cite{weberSourceinprep.}}
...
\begin{itemize}
    ...
    \item Il n'est pas pris en compte la spécificité du site en question (source atypique, topographie, climat...) ni la possible évolution du PO intrinsèque des sources qui serait lié à une
        évolution du profil chimique (renouvellement de parc automobile, etc.).
\end{itemize}
...
\todo{(=>  attention c'est pas une espèce chimique qui évolue, mais c'est une propriété liée à chaque particule qui va quand même évoluer en fonction de l'évolution (vieillissement, réaction chimique) de la particule) }, son
...
apporté par ces sources.\todo{ Et si c'est pas possible? plan c ou pas de plan c?}
...
\todo{=> ne parler du deuxième cas que si vous avez les résultats, sinon c'est trop prospectif}
...
\todo{  => tout  ça tu l'as déjà dit au début du chapitre...;}
