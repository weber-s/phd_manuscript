...
D'autres espèces ou groupement d'espèces permettent également le raffinement des PMF
...
stage d'ingénieur ; Kévin Asslanian, stage de M2 ; Nicole Montégnégro, stage de M1 ;
participation active au suivi de la thèse de Valéria Mardonez ; stage de M2 de
...
\begin{figure}[ht]
    ...
    de \PMdix{} (PMF et radiocarbone)]{Contributions relatives au carbone total en moyennes hivernales des sources
...
couplage entre PMF et météorologie, utilisant des données d'intensité 
...
courant que les aérosols proviennent de sources non locales, limitant l'intérêt de
cette méthode.
...
chacune des grilles.  Le ratio du nombre de trajectoires associées à une forte concentration
aux coordonnées $i$, $j$, noté $m_{ij}$, par le nombre total de trajectoires étant passées
...
\begin{figure}[ht]
    ...
    \caption[Illustration de la méthode PSCF]{Illustration de la méthode PSCF : les rétrotrajectoires depuis le site de
...
comme variable d'entrée des études PMF, quelles que soient leurs localisations. En effet, le
...
\begin{figure}[ht!]
    ...
    \caption[PSCF pour la provenance du MSA]{Probabilité de l'origine géographique du MSA, issue de l'article
...
\begin{tcolorbox}[colback=red!5!white,colframe=Melon,title=Note]
    ...
    facteurs PMF identifiés, mais les incertitudes des contributions temporelles ne sont pas
    facilement accessibles.

    Aussi, devant analyser de nombreux résultats PMF, j'ai simplifié leur traitement
    et le chargement des résultats au sein du module python
...
\begin{figure}[ht]
    ...
    \caption[Estimation de l'incertitude temporelle des concentrations des espèces sur un
    facteur PMF]{Incertitudes temporelles des concentrations de \PMdix, \NOt{} et OC* dans le
...
coefficient de correlation. Ainsi, si seul l'OC est similaire pour les deux profils mais
certains des métaux présentent des concentrations très différentes, alors le PD pourra tout de
même être acceptable car l'OC présente des concentrations relatives bien supérieures à celles
...
\begin{figure}[ht]
    ...
    \caption[Explication de la mesure de Pearson et influences des espèces dominantes]{Mesure de la distance de Pearson de deux sources \textit{wood burning}. L'image
...
Identity Distance} SID). Sa formulation a légèrement évolué entre son expression
...
\begin{figure}[ht]
    ...
    \caption[Illustration schématique de la distance SID]{Illustration schématique de la distance SID. Légende originelle:
...
par exemple des résultats de PMF absents de la base de données SPECIEUROPE.
...
\begin{tcolorbox}[colback=red!5!white,colframe=ProcessBlue,title=Note]
    ...
    partie en~\ref{annexe:INACS}) et lors d'une présentation orale à l'EAC de Tours en 2016
...
Dans de nombreuses PMF, il est rapporté un facteur secondaire inorganique contenant une
...
Une partie du rapport établi suite à cette étude est disponible en~\ref{annexe:INACS} et
ne sont présentés ci-après que les étapes et les résultats majeurs.
...
\begin{align}
    \dN(\NHq)_{atm} \times [\NHq]_{atm} &= \sum_{i=1}^{n} \dN(\NHq)_i \times [\NHq]_i
    \label{eq:melangeConcentration}
\end{align}
avec $[\NHq]_{atm} = \sum_{i=1}^n [\NHq]_i$ en \si{\ugm} la concentration atmosphérique
d'ammonium résultant du mélange des $n$ sources.  L'équation~\ref{eq:melangeConcentration}
peut alors être réécrite en considérant le ratio de la contribution de la source $i$ au
mélange final $f_i = [\NHq]_i / [\NHq]_{atm}$~\si{[-]}. Ainsi on a
\begin{align}
    \dN(\NHq)_{atm} &= \sum_{i=1}^n \dN(\NHq)_i \times f_i. 
    \label{eq:melangeContrib}
\end{align}

Selon certaines hypothèses, détaillées dans l'\ref{annexe:INACS}, en considérant
...
\begin{numcases}{\hspace{-2ex}\label{eq:system1}}
    1 &$= f_{bio} + f_{agr} + f_{veh}$ \label{eq:bilanMass1} \\
    \dN(\NHt)_{atm} &$= f_{bio} \times \dN(\NHt)_{bio} + f_{agr} \times \dN(\NHt)_{agr} +
    f_{veh} \times \dN(\NHt)_{veh}$\label{eq:systemNH4}
...
Deux nouvelles contraintes pourraient être ajoutées pour amener de l'information et
...
CHIANTI (voir section~\ref{sec:harmonisation_et_gestion_de_base_de_donnée}), il a été possible
...
\begin{numcases}{\hspace{-2ex}\label{eq:system3}}
    f_{bio} &$= \{\mathcal{N}(m_{bio},\sigma_{bio})\}$ / [\NHq$_{total}$] \label{eq:bioChimie}\\
    1 &$= f_{bio} + f_{agr} + f_{veh}$ \label{eq:bilanMasse}\\
    \dN(\NHt)_{atm} &$= f_{bio} \times \dN(\NHt)_{bio} + f_{agr} \times \dN(\NHt)_{agr} + f_{veh} \times \dN(\NHt)_{veh}$\label{eq:isotopie}.
\end{numcases}
...
\begin{figure}[ht]
    ...
    \caption[Estimation du ratio \NHq/lévoglucosan pour les jours de fortes concentrations en
        lévoglucosan]{Estimation du ratio \NHq/lévoglucosan pour les jours de fortes concentrations en
...
\begin{figure}[ht]
    ...
    \caption[Contribution temporelle des sources au \NHq{} sur le site de l'OPE]{Série
        temporelle des sources de \NHq{} déterminées par méthode isotopique pour le site de
        l'OPE. Sont représentés la moyenne de la densité
...
de Turin semblent être au-dessus de ces tendances et sont discutés en~\ref{annexe:INACS}.
...
\begin{figure}[ht]
    ...
    \end{center}
    \caption[Contribution moyenne de la source véhiculaire au \NHq{} durant les jours de la période chaude.]{Contribution moyenne de la source véhiculaire durant les jours de la période chaude.}
    \label{fig:summerNH4}
    \end{subfigure}
    \begin{subfigure}[t]{0.5\textwidth}
    \begin{center}
    ...
    \end{center}
    \caption[Contribution relative moyenne de la source agricole au \NHq{} pour les jours des trois
    pics de pollution printaniers observés.]{Contribution relative moyenne de la source agricole pour les jours des trois
    pics de pollution printaniers observés.}
    \label{fig:contribSpringAgr}
    \end{subfigure}
    \caption{Contributions des sources véhiculaire et agricole au \NHq{} (OPE).}
\end{figure}
...
\begin{table}[ht]
    ...
    fraction \PMdc{} pour le site de l'OPE.}
    \label{tab:varPMF}
\end{table}
...
\begin{figure}[ht]
    ...
    \caption[Profils PMF pour une solution à 8 facteurs sur le site de l'OPE.]{
...
\begin{figure}[ht]
    ...
    \caption[Reconstruction des variables \NHq$_{src}$, \NOt{} et \NHq$_{tot}$ d'après
    l'étude PMF sur le site de l'OPE]{Reconstruction des variables \textbf{a)} \NHq$_{src}$, \textbf{b)} \NOt~et
...
considérées. Cette limitation est davantage explorée dans l'\ref{annexe:INACS}.
...
\begin{figure}[ht]
    ...
    \caption[Distribution spatio-temporelle des concentrations de polyols (moyenne et écart
        type) en \si{\ngm}.]{
...
avec les labours, confortent encore une fois la provenance biogénique, mais précise cette
fois le compartiment en question : les polyols proviennent très certainement des plantes
et non du sol.

Pour s'en assurer, une troisième étude, analysant l'ADN de prélèvements d'échantillons
...
\begin{figure}[ht]
    ...
    \caption[Évolution conjointe des concentrations de polyols, cellulose, glucose et des
...
partie d'aérosol organique biogénique secondaire. En l'absence d'espèces tracant les
processus secondaires (comme le 3-MBTCA ou l'acide pinic), cette possibilité reste
ouverte.  Cette hypothèse parait cependant a priori peu probable. En effet, le ratio OC/polyols du
profil PBOA est relativement peu variable entre les sites, alors qu'il serait peu
probable que la variabilité des processus secondaires conduise à une concentration d'OC
identique sur l'ensemble de ces sites.  Aussi, les ratios OC/polyols des profils PBOA
(autour de 16) sont bien dans la gamme attendue de variabilité des mesures pour les spores de champignons
...
\begin{figure}[ht]
    ...
    \caption[Contribution des polyols à la masse de l'OM du facteur PBOA, et contributions de
        l'OM du PBOA à la masse total de l'OM des \PMdix{} sur les 16 sites étudiés]{
        Contribution des polyols à la masse de l'OM du facteur PBOA, et contributions de
        l'OM du PBOA à la masse total de l'OM des \PMdix{} sur les 16 sites étudiés
        (moyenne annuelle et estivale).
        Les étoiles et cercles représentent respectivement les sites urbains sans et avec
        influence des vallées alpines. Les pentagones correspondent au site trafic, et
        les diamants aux sites ruraux.
...
dans ce facteur reste ouverte, en l'absence (dans ces études de synthèse) d'espèces
traceuses de processus secondaire
...
permet des travaux sur la capacité de la méthode PMF à saisir la variabilité fine échelle
de l'ensemble des sources identifiées.
...
\begin{tcolorbox}[colback=red!5!white,colframe=Melon,title=Note]
    ...
    Ce projet d'article a été envoyé aux co-auteurs début août 2020, et leurs commentaires n'y
    figurent pas encore.

    L'intégralité du papier est disponible en~\ref{annexe:borlazaSA}.
\end{tcolorbox}
...
\begin{figure}[ht]
    ...
    \caption[Évolutions temporelles des acides organiques, cellulose et autres espèces sur les trois
    sites Grenoblois]{
...
discutées plus en avant dans l'article proposé en~\ref{annexe:borlazaSA}.
...
sources de PM, à travers des approches site-recepteurs -- et donc PMF -- afin d' être au plus
...
incluant :
\begin{itemize}
    \item Une estimation des incertitudes des mesures chimiques communes,
...
\begin{tcolorbox}[colback=red!5!white,colframe=Melon,title=Note]
    ...
    Le complément d'information est disponible sur \url{http://pmsource.u-ga.fr} et en~\ref{annexe:SOURCES_SI}.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=red!5!white,colframe=Melon,title=Note]
    ...
    Au 19 août 2020, ce site a déjà été visionné par plus de 400 personnes provenant
...
\includepdf[pages=-,scale=0.93,pagecommand={\pagestyle{fancy}}]{chapters/sources.pdf}
...
les sources prépondérantes de \PMdix{} à grande échelle lors d'études PMF multi-sites.
