...
\begin{figure}[ht]
    ...
    \caption[Comparaison des variables atmosphériques selon l'atmosphère standard]{%
...
dans la troposphère que les phénomènes météorologiques auxquels nous sommes habitués se
...
\begin{figure}[h]
    ...
    \caption[Évolution journalière de la couche limite atmosphérique]{Évolution journalière schématique de la couche limite atmosphérique.
        Credit: By
...
\begin{figure}[ht]
    ...
    \caption[Distribution en nombre, surface et volume d'un ensemble typique trimodale
    d'aérosols]{Distribution \textbf{(a)} en nombre, \textbf{(b)} surface, et
...
\begin{itemize}
    ...
    \item \PMun, parfois appelées particules ultrafines (UFP), dont le diamètre
        ...
        vapeur…)
\end{itemize}
Par définition, les \PMdix{} contiennent donc les autres modes, et ainsi de suite.
...
\begin{figure}[ht]
    ...
    \caption[Diversité des aérosols observée au microscope électronique à balayage]{Image au microscope électronique à balayage, à des échelles différentes,
...
\begin{figure}[ht]
    ...
    \caption[Composition chimique majeure de la masse des \PMdix{} à l'échelle
    planétaire]{Composition chimique majeure de la masse des \PMdix{} pour différentes
...
des aérosols.  Enfin, le dépôt des aérosols et notamment du \textit{black carbon} sur la
...
de \SI{2.29}{\W\per\m\squared}), avec un forçage négatif pour
...
étaient revenus à leurs états antérieurs à décembre 1952 \autocite{bellReassessment2001}.  Ces
...
\begin{figure}[ht]
    ...
    \caption[Évolution temporelle des émissions cumulées des 28 pays de la zone
    européenne depuis 1990]{Évolution temporelle des émissions cumulées des 28 pays de la zone européenne
...
Chimère\footnote{Chimère : \url{https://www.lmd.polytechnique.fr/chimere/}},
LOTOS-EUROS\footnote{LOTOS-EUROS : \url{https://lotos-euros.tno.nl/}}...
...
inventoriée, etc.
En plus de cette connaissances de la quantité des émissions, celle de leurs distributions
temporelles et spatiales s'avère également compliquée. Notamment, la répartition annuelle
...
Aussi, la limitation physique des modèles météorologiques se retrouve dans les CTM. Il
...
françaises de PREV'AIR \parencite{rouilPrev2009} se font à résolution spatiale de l'ordre
...
\begin{figure}[ht]
    ...
    \caption[Connaissances a priori nécessaires pour les différents modèles de SA existant]{
...
\begin{figure}[ht]
    ...
    \caption[Différents types d'apprentisages entre PMF et ACP sur une banque d'image de
    portraits]{Analyse factorielle par PMF (ou NMF pour \textit{non-negative matrix
...
La première incertitude est évaluée à travers une méthode dit de \textit{bootstrap} consistant
...
L'ajout de ces contraintes limite de fait les rotations possibles des matrices $F$ et
...
différentes études, comme le montrent les z-scores de la figure~\ref{fig:chapter01/belisEvaluation2020_fig2a}
...
\begin{figure}[ht]
    ...
    \caption[Comparabilité des séries temporelles obtenues par les 38 différentes solutions sur l'étude de comparaison des modèles sources-récepteurs sur le site de Lens]
    {Comparabilité des séries temporelles obtenues par les 38 différentes solutions
...
\begin{table}[!ht]
    \begin{ThreePartTable}
        ...
        \footnotesize
        \begin{tabular}{cp{0.41\textwidth}p{0.41\textwidth}}
        \toprule
        ...
        \begin{itemize}[topsep=0pt, left=0pt, label={\unicodesymbols ✘}]
          ...
          \item Ne voit que ce qui est donné des cadastres
          \item Variabilité temporelle des émissions peu robuste
          \item Nombre d'espèces chimiques limité
        \end{itemize}
        \\ \midrule
        CMB & 
        \begin{itemize}[topsep=0pt, left=0pt, label={\unicodesymbols ✔}]
          \item Spécificité du site potentiellement prise en compte
          \item Grand nombre d'espèces pouvant être utilisées
        \end{itemize}
            & 
        \begin{itemize}[topsep=0pt, left=0pt, label={\unicodesymbols ✘}]
          \item Faible représentativité spatiale et temporelle
          \item Prélèvement et analyses couteux (humain et financier)
          \item Profils des sources fixes et connus à l'avance
          \item Nécessite une base de connaissance de profils chimiques
        ...
        PMF-filtre &
        \begin{itemize}[topsep=0pt, left=0pt, label={\unicodesymbols ✔}]
          \item Large gamme de famille d'espèces chimiques
          \item Directement applicable pour la réglementation
          \item D'autres analyses possibles sur les mêmes filtres (potentiel oxydant par
              exemple)
        \end{itemize}
            & 
        % \begin{itemize}[topsep=0pt, left=0pt, label={\unicodesymbols ✘}]
...
utilisées. La détermination d'un facteur sera grandement facilitée par la présence d'une
...
espèces métalliques et la présence de potassium, mais l'ajout du lévoglucosan -- produit de
pyrolyse de la cellulose -- permet une identification beaucoup plus claire et non équivoque
de ce facteur.

L'utilisation d'\textit{espèces traceuses} rend l'identification de facteur plus aisée,
mais permet aussi la détermination de nouvelles sources qui auraient étées jusqu'alors peu ou
pas visibles.
Par exemple, \cite{srivastavaSpeciation2018a} ont pu déterminer des facteurs peu
courrants
grâce à l'ajout de certaines espèces organiques traceuses de différents processus :
...
DHOPA).
De même, l'analyse des HULIS a permis une meilleure déterminantion des facteurs organiques.
\cite{chevrierChauffage2016} a également utilisé des mesures de radiocarbone et la
séparation en carbone \textit{fossil fuel} et \textit{wood burning} provennant de mesures
d'aethalomètre AE33, permettant une séparation et une évaluation des
contributions des facteurs plus précises grâce à ces traceurs spécifiques.

Cependant, bien que l'on sache maintenant l'importance de l'ajout de ces espèces dans les
PMF, la généralisation de l'emploi de ces traceurs est limitée par le coût analytique
et l'automatisation de certaines de ces mesures.
Il convient donc d'essayer de déterminer le corpus analytique permettant une
caractérisation chimique optimisée à un coût raisonnable.
...
pas le même impact sur notre santé qu'un~\si{\ugm} de mercure ou de plomb.  Seulement,
la
...
pour lesquels la mesure de la concentration massique est tout à fait adaptée.
...
\begin{figure}[ht]
    ...
    \caption[Mécanismes de toxicité de l’ozone et des particules atmosphériques dans les voies aériennes]
    {Mécanismes de toxicité de l’ozone et des particules atmosphériques dans les voies
    aériennes. Crédit : \cite[figure 1]{baezaPollution2007}}%
    \label{fig:mecanisme_oxydation}
\end{figure}

Afin de répondre à cette problématique de mesure de l'impact sanitaire des PM, il a été proposé
...
différentes. Au cours de cette thèse, deux mesures différentes sont utilisées conjointement :
...
D'autres méthodes de mesures du PO existent, mais n'ont pas été utilisées au cours de
...
\begin{description}
    ...
    \item[Mélange d'anti-oxydant (RTLF)] Bien que la plupart du temps les différents
        anti-oxydants soient testés indépendamment, il est
...
Cependant, tous les métaux ne réagissent pas de façon similaire et d'autres comme le fer,
le zinc ou le chrome ne présentent pas de réactivité particulière au test du DTT.
...
intrinsèque à chacune des espèces chimiques.
...
PO, mais se révèle largement insuffisante.
...
simplement en terme de réglementation de la qualité de l'air et permet
...
directement à l'émission. Les sources les plus échantillonnées sont différentes émissions
liées aux trafic routier \parencite{biswasOxidative2009,parkDifferential2018} ou à l'industrie
\parencite{jooPhysicochemical2018}, ainsi que la combustion de biomasse
...
Les sources d'émissions liés aux émissions primaires du trafic semblent avoir un \PODTTm{}
...
De plus, de nombreux secteurs d'émissions doivent déjà présenter des prélèvements avec
une
caractérisation chimique (banc d'essai, industrie, etc). 
Il est également probable que de nombreux programmes passés de caractériation des émissions permettraient
...
différentes :
\begin{enumerate}
    \item Incorporation des POs comme variables d'entrée d'une étude PMF au même titre qu'une
...
facteurs secondaires (organique et inorganique).
...
\begin{figure}[ht]
    ...
    \cite{fangOxidative2016}]{Contribution des sources de PM aux \POAAv{} et \PODTTv{} par l'étude de
...
mêmes que c'est une question centrale qui intéresse d'autant plus les décideurs publics.
Mes travaux s'appliqueront donc à systématiquement quantifier les incertitudes des
...
et potentiel oxydant, qui ont commencé à être mises en évidence par les études
antérieures, manquent de généralisation spatiale, les études précédentes ne couvrant que peu
...
\begin{itemize}
    ...
    \item Est-ce que les valeurs PO des sources des PM sont les mêmes à large échelle spatiale ou
        ...
        (Chapitre~\ref{cha:estimation_des_sources_de_PO})
    \item Est-il possible, à terme, d'obtenir une prévision du PO intégrée à la prévision
...
La liste complète de ces travaux est fournie en~\ref{annexe:CV} et s'appuie sur un
